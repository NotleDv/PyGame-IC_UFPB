...
def main():
    ...
    def jogadas(click_point, total_jogadas, max_jogadas, history_points, jogada, status_bar):
        ...
        if player_atual != player_anterior:
            ...
            ## Verifica se o element está matriz do game
            qtd_bau, status, element, validacao = click(click_user=click_point, matriz=matriz_k,search_elements=search_elements)
            
            """ Validação retorna se o element é valido, se for valido e o tempo tiver acabado então é passado a vez e não é contado nada"""
            if validacao:
                if status_bar['termino'] == True:
                    troca_jogador(jogada)
                    time_play()
                    return
            
            ## Verificar se o element já foi clicado     
            validacao_2 = False
    ...
    ###############
    def time_play():
        """Thread que controla a barra de progresso do tempo do jogador"""
        status_bar['termino'] = False
        stop_event.clear()   # reset da flag
        for i in range(530, 0, -1):
            if stop_event.is_set():  # se foi interrompido
                return
            status_bar["progresso"] = i
            pygame.time.delay(20)
        status_bar["termino"] = True  # acabou o tempo
    
        
    def troca_jogador(jogada):
        """Troca os jogadores"""
        jogada['player_atual'], jogada['player_anterior'] = jogada['player_anterior'], jogada['player_atual']
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                total_jogadas = jogadas(posicao_click, total_jogadas, max_jogadas, history_points, jogada, status_bar)
                stop_event.set()
                print(total_jogadas)                
                                        
        display.blit( surface_head, (0, 0) ) 
        
        # verifica se o timer acabou sem clique
        if status_bar['termino']:
            status_bar['termino'] = False
            troca_jogador(jogada)
        ...
        pygame.draw.rect(surface_head, color_barra, (170, 20, largura, 25))
        front_progresso(surface_head, rect_prog) 
